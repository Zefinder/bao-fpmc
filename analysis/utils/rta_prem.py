...
def get_memory_phase_start_time(system: PREM_system, interference_mode: inter_processor_interference_mode, cpu_prio: int, Px: processor, task: PREM_task, k: int) -> int:
    ...
    if (cpu_prio == 1):
        print('Base mem start time:', start_time, 'k:', k)
    prev_start_time = -1
...
def get_computation_phase_start_time(system: PREM_system, interference_mode: inter_processor_interference_mode, cpu_prio: int, Px: processor, task: PREM_task, k: int) -> int:
    ...
    if (cpu_prio == 1):
        print('Mem start time:', memory_start_time, '\n')
    ...
    while prev_start_time != start_time:
        ...
        if (cpu_prio == 1):
            print("alpha(s_cmp):", inter_processor_interference)
            print("alpha(s_cmp - s_mem):", inter_processor_interference_mid)
            print("beta(s_mem):", beta_memory_phase)
            print("B + I + M:", constant_blocking)
            print("s_cmp:", start_time)
            print()
...
def get_longest_busy_period(system: PREM_system, interference_mode: inter_processor_interference_mode, cpu_prio: int, Px: processor, task: PREM_task) -> int:
    ...
    while prev_busy_period != busy_period:
        ...
        if (cpu_prio == 1):
            print("I:", intra_processor_interference)
            print("alpha:", interference_mode.get_inter_processor_interference(system=system,
                                                                     cpu_prio=cpu_prio,
                                                                     delta=busy_period,
                                                                     task=task))
            print("beta:", get_total_memory_interference(system=system,
                                                interference_mode=interference_mode,
                                                cpu_prio=cpu_prio,
                                                Px=Px,
                                                task=task,
                                                delta=busy_period))
            print("M_max:", Px.M_max)
            print("L:", busy_period)
            print()
